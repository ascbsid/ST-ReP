...
def fit_ridge(logger, train_features, train_y, valid_features, valid_y, MAX_SAMPLES=100000):
    ...
    if train_features.shape[0] > MAX_SAMPLES:
        split = train_test_split(
            ...
            train_size = MAX_SAMPLES, random_state=0, shuffle=True
        )
    ...
    for alpha in alphas:
        ...
        logger.info(f'alpha:{alpha}, results:{score}')
...
def get_rep(model, data, seq_len, batch_size, device, seq_samples_done=False):
    if seq_samples_done:
        dataset = naiveDataset(data)
    else:
...
def generate_pred_samples_forHL(feature, data, seq_len, pred_len):
    # logger.info(f'in generate samples: data_x.shape {data_x.shap}, data_repr.shape {data_repr.shape}')
...
def eval_HL(logger, expid, args, data_list, scaler, ds_pred_len_list, log_csv_path, save_path):
    ...
    for i, pred_len in enumerate(ds_pred_len_list):
        ...
        result_norm_txt_path = f'./results/{args.modelid}/{args.modelid}_{args.dataset_name}_Results_norm.txt'
        result_norm_csv_path = f'./results/{args.modelid}/{args.modelid}_{args.dataset_name}_Results_norm.csv'
        
        cal_metric_log(logger, expid, args, yhat, realy, pred_len, None, None, None, None,
                            result_norm_csv_path, result_norm_txt_path, log_csv_path=None, save_path=save_path, flag='np')
...
def eval_forecasting_ts(logger, expid, args, model, data_list, device, scaler, predict_length_list,
                        log_csv_path, model_save_path):
    ...
    input_length = args.ds_input_length
    seq_samples_done = len(test_data[0].shape)==4


    if useRep:
        t = time.time()
        all_train_repr = get_rep(model, [train_data[0],train_data[1]], seq_len=input_length, batch_size=1,device=device, seq_samples_done=seq_samples_done)
        all_val_repr = get_rep(model, [val_data[0],val_data[1]], seq_len=input_length, batch_size=1, device=device, seq_samples_done=seq_samples_done)
        all_test_repr = get_rep(model, [test_data[0],test_data[1]], seq_len=input_length, batch_size=1, device=device, seq_samples_done=seq_samples_done)
        rep_infer_time = time.time() - t
        logger.info(f'rep_infer_time:{rep_infer_time}')
        logger.info(f'all_train_repr.shape:{all_train_repr.shape}') #B 1 N d
    else:
    ...
    for predict_length in predict_length_list:
        ...
        if seq_samples_done:
            train_repr, train_labels = all_train_repr, train_data[2]
            val_repr, val_labels = all_val_repr, val_data[2]
            test_repr, test_labels = all_test_repr, test_data[2]
        else:
            if useRep:
        ...
        cal_metric_log(logger, expid, args, test_pred, test_labels, predict_length,
                            lr_train_time,lr_infer_time, None, None,
                            result_norm_csv_path, result_norm_txt_path, log_csv_path, model_save_path, flag='np')    
