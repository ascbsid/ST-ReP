...
parser.add_argument('--pred_hasRes', type=int, default=1)

# for the downstream tasks
parser.add_argument('--ds_modelid', type=str, default='ridge')
parser.add_argument('--ds_input_length', type=int, default=-1)
parser.add_argument('--ds_predict_length', type=int, default=-1)
...
parser.add_argument('--yita', type=float, default=0.3)
...
class Exp():
    ...
    def _build_loss_func(self, loss_type='pred_mask_ms', delta=1.0):
        if loss_type == 'pred_only':
            ...
            def loss_func(inputx, reconx, inputy, predy):
                ...
                full_loss = (1-self.beta-self.yita)*pred_loss + self.beta*reconx_loss + self.yita*msloss
                return full_loss
            return loss_func
    ...
    def train(self, logger, train_dataloader, val_dataloader, model_save_path):
        ...
        for i in range(1, args.epochs + 1):
            ...
            for iter, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_dataloader):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.float().to(self.device)
                batch_x_mark = batch_x_mark.int().to(self.device)
                batch_y_mark = batch_y_mark.int().to(self.device)
                trainx = [batch_x, batch_y, batch_x_mark, batch_y_mark]
            ...
            s1 = time.time()
            for iter, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(val_dataloader):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.float().to(self.device)
                batch_x_mark = batch_x_mark.int().to(self.device)
                batch_y_mark = batch_y_mark.int().to(self.device)
                valx = [batch_x, batch_y, batch_x_mark, batch_y_mark]
...
def main():
    ...
    if os.path.exists(f'./configurations/{args.config_file}'):
        with open(f'./configurations/{args.config_file}', 'r') as f:
            ...
            args.root_path = data_config['root_path']
            ...
            args.ds_input_length = data_config['ds_input_length']
            args.ds_predict_length = data_config['ds_predict_length']
            args.num_workers = 10

            if model_config['modelid']!='default': args.modelid = model_config['modelid']
            ...
            args.tau = model_config['tau']
            args.time_factor = model_config['time_factor']

            args.yita = training_config['yita']
            args.beta = training_config['beta']
    ...
    if args.input_length == args.ds_input_length and args.predict_length == args.ds_predict_length :
        data_list =[
        ...
        ds_train_dataset, ds_train_dataloader = data_provider(args, flag='train', forDownstream=True)
        ds_val_dataset, ds_val_dataloader = data_provider(args, flag='val', forDownstream=True)
        ds_test_dataset, ds_test_dataloader = data_provider(args, flag='test', forDownstream=True)
    ...
    if args.modelid in ['Ridge','HL'] or bool(args.useRep)==False:
        ...
        if args.load_expid != 'none':
            load_path = f'./experiments/{args.modelid}/{args.dataset_name}/{args.modelid}_{args.dataset_name}_{args.input_length}to{args.predict_length}_exp{args.load_expid}/best.pth'
    ...
    if args.modelid == 'HL':
        forecasting.eval_HL(logger=logger, expid=expid, args=args, data_list=data_list, scaler=ds_scaler, 
                             ds_pred_len_list=args.ds_pred_len_list, 
                             log_csv_path = None, save_path=save_path)    
        ...
        forecasting.eval_forecasting_ts(logger=logger, expid=expid, args=args, model=engine.model, data_list=data_list, 
                                                ...
                                                log_csv_path = None, model_save_path=save_path)
    else:
